...
class Trainer:
    ...
    def make_grid(self, batch, gen, output_dir, name, batches_done):
        img_grid = torch.cat(((batch['LR'][tio.DATA][:10].squeeze(4) * self.std).to(self.device),
                              (batch['HR'][tio.DATA][:10].squeeze(4) * self.std).to(self.device),
                              torch.clamp((gen[:10] * self.std), 0, 1),
                              (batch['LR'][tio.DATA][:10].squeeze(4) * self.std).to(self.device) - (gen[:10] * self.std) + .5), -1)
    ...
    def training(self, train_batch):
        self.netG.train()
        self.netD.train()
        imgs_lr = Variable(train_batch['LR'][tio.DATA].squeeze(4).type(self.Tensor))
        imgs_hr = Variable(train_batch['HR'][tio.DATA].squeeze(4).type(self.Tensor))
        ...
        if self.lambda_pixel is not 0:
            # Measure pixel-wise loss against ground truth
            loss_pixel = self.criterion_pixel(gen_hr, imgs_hr)
        else:
            loss_pixel = torch.tensor([0.0]).to(self.device)

        if self.lambda_adv is not 0:
            ...
            loss_GAN = torch.tensor([0.0]).to(self.device)

        if self.lambda_content is not 0:
            # Content loss
            ...
            loss_content = torch.tensor([0.0]).to(self.device)

        if self.lambda_edge is not 0:
            # Edge loss
    ...
    def validate(self, validation_batch):
        self.netG.eval()
        self.netD.eval()
        imgs_lr = Variable(validation_batch['LR'][tio.DATA].squeeze(4).type(self.Tensor))
        imgs_hr = Variable(validation_batch['HR'][tio.DATA].squeeze(4).type(self.Tensor))
