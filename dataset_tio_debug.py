...
class ImagePair(object):
    ...
    def subject(self):
        ...
        if self.select_slices is not None:
            ...
            LR = self.LR.get_fdata()#[:,:,::2]
            HR = self.HR.get_fdata()#[:,:,::2]
            LR_msk = self.LR_msk.get_fdata()#[:,:,::2]
            HR_msk = self.HR_msk.get_fdata()#[:,:,::2]
            HR_msk_binary = self.HR_msk_binary.get_fdata()#[:,:,::2]
            HR_msk_binary[HR_msk_binary>0]=1
            HR_msk_binary = cv2.erode(HR_msk_binary, np.ones((10, 10)), iterations=3)
...
class RealImage(object):
    ...
    def fnames(self):
        ...
        return LRf, NNf, CBf, GTf, MSKf

    def to_nifty(self):
        LR_fname, NN_fname, CB_fname, GT_fname, MSK_fname = self.fnames()
        self.LR = nib.load(LR_fname)
    ...
    def subject(self):
        ...
        if self.select_slices is not None:
            ...
            CB = self.CB.get_fdata()-self.CB.get_fdata()[0,0,0]
